...
// ===== Config =====
const API_BASE = "https://asistente.ponganos10.online"; // tu backend
...
els.openCalendar.addEventListener("click", () => {
  prefillCalendarTimes();
  els.calModal.showModal();
});
...
function appendMessage(role, text) {
  ...
  li.innerHTML = `
    ...
    <div class="meta">${role === "user" ? "Tú" : "Agente"}</div>
  `;
...
async function sendEmail(e) {
  ...
  try {
    const res = await fetch(GMAIL_URL, {
      ...
      credentials: "include", // por si el backend administra sesión
    });
    const data = await res.json();
...
async function createEvent(e) {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) throw new Error(data.detail || `HTTP ${res.status}`);
    appendMessage("ai", `✅ Evento creado: ${payload.summary} (${payload.start} → ${payload.end})`);
    els.calModal.close();
  } catch (err) {
...
function localDateTimeToISO(localValue) {
  // Convierte "yyyy-MM-ddTHH:mm" a ISO con zona local
